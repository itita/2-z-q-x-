...
#define nameStr "摩托化步兵"
#define rarity 3

#define oriHealth 300
#define oriDamage 80
#define oriCost 100
...
UnitBean* Infanterie::unitBean = NULL;

MyUnit* Infanterie::createUnit(int level) {
...
//    CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect(sfxDeployNameStr);
    ...
    unit->moveAnimateCreator = (SEL_UnitFXCreator)(&Infanterie::getMoveAnimate);
    unit->fireAnimateCreator = (SEL_UnitFXCreator)(&Infanterie::getFireAnimate);
    unit->dieAnimateCreator = (SEL_UnitFXCreator)(&Infanterie::getDieAnimate);
...
CCAnimate* Infanterie::getMoveAnimate(int dir){
    char str[40] = {0};
    if (dir == 0) {
...
CCAnimate* Infanterie::getDieAnimate(int dir){
    char str[40] = {0};
    if (dir == 0) {
...
UnitBean* Infanterie::getUnitBean() {
    if (unitBean == NULL) {
        unitBean = UnitBean::create(InfanterieCardId,CCString::create(nameStr),CCString::create(spriteFileNameStr),baseCost,updateCostRate,rarity);
        unitBean->createUnit_selector = (SEL_CreateUnit)(&Infanterie::createUnit);
        unitBean->getUnitCardInfo_selector = (SEL_GetUnitCardInfo)(&Infanterie::getUnitCardDesc);
        unitBean->getAttackSprite_selector = (SEL_GetAttackSprite)(&Infanterie::getAttackSprite);
...
CCLayer* Infanterie::getUnitCardDesc(int level,bool isAll) {
...
CCSprite* Infanterie::getAttackSprite(int dir) {
    CCSprite* army;
    CCAnimate* action;
    char str[50] = {0};
    if (dir == attackLD) {
