...
UnitBean* M3HalfTrack::unitBean = NULL;

MyUnit* M3HalfTrack::createUnit(int level) {
...
//    CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect(sfxDeployNameStr);
    ...
    unit->moveAnimateCreator = (SEL_UnitFXCreator)(&M3HalfTrack::getMoveAnimate);
    unit->fireAnimateCreator = (SEL_UnitFXCreator)(&M3HalfTrack::getFireAnimate);
    unit->dieAnimateCreator = (SEL_UnitFXCreator)(&M3HalfTrack::getDieAnimate);
...
CCAnimate* M3HalfTrack::getMoveAnimate(int dir){
    char str[40] = {0};
    if (dir == 0) {
...
CCAnimate* M3HalfTrack::getDieAnimate(int dir){
    char str[40] = {0};
    if (dir == 0) {
...
UnitBean* M3HalfTrack::getUnitBean() {
    if (unitBean == NULL) {
        unitBean = UnitBean::create(M3HalfTrackCardId,CCString::create(nameStr),CCString::create(spriteFileNameStr),baseCost,updateCostRate,rarity);
        unitBean->createUnit_selector = (SEL_CreateUnit)(&M3HalfTrack::createUnit);
        unitBean->getUnitCardInfo_selector = (SEL_GetUnitCardInfo)(&M3HalfTrack::getUnitCardDesc);
        unitBean->getAttackSprite_selector = (SEL_GetAttackSprite)(&M3HalfTrack::getAttackSprite);
        unitBean->unlockNeedCount = 3025001;
...
CCLayer* M3HalfTrack::getUnitCardDesc(int level,bool isAll) {
...
CCSprite* M3HalfTrack::getAttackSprite(int dir) {
    CCSprite* army;
    CCAnimate* action;
    char str[50] = {0};
    if (dir == attackLD) {
