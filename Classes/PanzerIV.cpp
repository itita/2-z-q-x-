...
#define spriteFileNameStr "PanzerIV"
...
#define attackRate 0.7

#define baseCostCool 1.25

UnitBean* PanzerIV::unitBean = NULL;

MyUnit* PanzerIV::createUnit(int level) {
    ...
    unit->moveAnimateCreator = (SEL_UnitFXCreator)(&PanzerIV::getMoveAnimate);
    unit->fireAnimateCreator = (SEL_UnitFXCreator)(&PanzerIV::getFireAnimate);
    unit->dieAnimateCreator = (SEL_UnitFXCreator)(&PanzerIV::getDieAnimate);
...
CCAnimate* PanzerIV::getMoveAnimate(int dir){
    char str[40] = {0};
    if (dir == 0) {
...
CCAnimate* PanzerIV::getDieAnimate(int dir){
    char str[40] = {0};
    if (dir == 0) {
...
UnitBean* PanzerIV::getUnitBean() {
    if (unitBean == NULL) {
        unitBean = UnitBean::create(PanzerIVCardId,CCString::create(nameStr),CCString::create(spriteFileNameStr),baseCost,updateCostRate,rarity);
        unitBean->createUnit_selector = (SEL_CreateUnit)(&PanzerIV::createUnit);
        unitBean->getUnitCardInfo_selector = (SEL_GetUnitCardInfo)(&PanzerIV::getUnitCardDesc);
        unitBean->getAttackSprite_selector = (SEL_GetAttackSprite)(&PanzerIV::getAttackSprite);
...
CCLayer* PanzerIV::getUnitCardDesc(int level,bool isAll) {
...
CCSprite* PanzerIV::getAttackSprite(int dir) {
    CCSprite* army;
    CCAnimate* action;
    char str[50] = {0};
    if (dir == attackLD) {
