...
DefendQuest* DefendQuest::create() {
    DefendQuest* task = new DefendQuest();
    task->autorelease();
    //    task->power = _power;
...
bool DefendQuest::excuteMsg(MyBaseMessage* msg) {
    if (msg->tag == defendOverMessageTag) {
        if (((DefendOverMessage*)msg)->isVictory) {
            int defendTimesValue = defendTimes->getValue();
            defendTimesValue++;
            defendTimes->setValue(defendTimesValue);
        }
        char str[10] = {0};
        sprintf(str,"%d",defendTimes->getValue());
        MyHttpConnection::getInstance()->sendUpdateMissionProgress(str,questId);
    }
    return MyBaseQuest::excuteMsg(msg);
}

int DefendQuest::getCompleteStatus() {
...
CCLayer* DefendQuest::getPrizeInfo() {
...
CCLabelTTF* DefendQuest::getTitle() {
